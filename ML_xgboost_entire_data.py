...
print('INTITIAL NUMBER OF COLUMNS')
initial_data_cols = list(data.columns)
print(initial_data_cols)
print(len(initial_data_cols))
print()
...
columns_to_drop_idx_len = len(columns_to_drop_idx)
print('HERE: ', columns_to_drop_idx_len)
...
roc_auc = metrics.roc_auc_score(y_test_pc, clf.predict_proba(X_test_pc.values)[:,1])
print("ROC AUC is: ", roc_auc)

f1 = metrics.f1_score(y_test_pc, clf.predict(X_test_pc.values))
print("F1 score is: ", f1)

precision = metrics.precision_score(y_test_pc, clf.predict(X_test_pc.values))
print("Precision score is: ", precision)

recall = metrics.recall_score(y_test_pc, clf.predict(X_test_pc.values))
...
prec_test, recall_test, _ = metrics.precision_recall_curve(y_test_pc, preds[:,1])
# Calculate AUC-PRC
auc_prc_test = metrics.auc(recall_test, prec_test)
print("TEST Precision-Recall AUC:", auc_prc_test)

prec_val, recall_val, _ = metrics.precision_recall_curve(y_val_pc, preds_valid[:,1])
# Calculate AUC-PRC
...
plt.savefig('logs_MH_subset/figures/xgboost_entire_data_testPC_prec_recall_curve.png')
print()
print()

# ROC-AUC curve -- Test data
fpr_curve, tpr_curve, _ = metrics.roc_curve(y_test_pc, preds[:,1])
...
plt.savefig('logs_MH_subset/figures/xgboost_entire_data_testPC_ROC_curve.png')
...
df_pc.to_csv('logs_MH_subset/saved_csvs/xgboost_entire_data_testPC.csv')
...
# plt.savefig('logs_MH_subset/figures/xgboost_entire_data_testPC_SHAP_bar.png')
#
# plt.figure()
# shap.summary_plot(shap_values, X_test_pc.values, max_display=20)
# plt.savefig('logs_MH_subset/figures/xgboost_entire_data_testPC_SHAP_summary.png')
...
print('Train PC+Non PC, Val Non PC')
def objective(trial):
    n_estimators = trial.suggest_int("n_estimators", 200, 500)
    max_features = trial.suggest_categorical("max_features", ['sqrt', 'log2']) # NICK: removed 'auto' 3/7/24
    max_depth = trial.suggest_int('max_depth', 4, 50)
...
# plt.title('Precision-Recall Curve')
plt.legend(loc="lower left")
plt.savefig('logs_MH_subset/figures/xgboost_entire_data_testNonPC_prec_recall_curve.png')
print()
print()
...
# plt.title('Receiver Operating Characteristic')
plt.legend(loc="lower right")
plt.savefig('logs_MH_subset/figures/xgboost_entire_data_testNonPC_ROC_curve.png')
print('saved roc-auc curve')
print()
...
df_pc.to_csv('logs_MH_subset/saved_csvs/xgboost_entire_data_testNonPC.csv')
print('metrics file saved')
print()
...
# # SHAP
# explainer = shap.TreeExplainer(clf)
# shap_values = explainer.shap_values(X_test_non_pc.values)
# shap.plots.bar(shap_values, max_display=20) #, show=False)
# plt.savefig('logs_MH_subset/figures/xgboost_entire_data_testNonPC_SHAP_bar.png')
#
# shap.summary_plot(shap_values, X_test_non_pc.values, max_display=20)
# plt.savefig('logs_MH_subset/figures/xgboost_entire_data_testNonPC_SHAP_summary.png')
# print('shap done and saved')
# print()
