...
# ALL
female_list = data['female'].dropna().tolist()
female_all = data[(data['female'] == 1)]
male_all = data[(data['female'] == 0)]

# PC DATA
female_list = pc_data['female'].dropna().tolist()
female_pc = pc_data[(pc_data['female'] == 1)]
male_pc = pc_data[(pc_data['female'] == 0)]

# NON_PC DATA
female_list = non_pc_data['female'].dropna().tolist()
female_nonpc = non_pc_data[(non_pc_data['female'] == 1)]
male_nonpc = non_pc_data[(non_pc_data['female'] == 0)]


genders = [female_all, male_all]
genders_pc = [female_pc, male_pc]
genders_nonpc = [female_nonpc, male_nonpc]

gender_domains = [genders, genders_pc, genders_nonpc]
names = ['All', 'PC', 'Non-PC']


for d in range(0, len(gender_domains)):
    domain = gender_domains[d]
    print(names[d])
    # print(domain)
    counts_1 = []
    counts_0 = []
...
print('PC:')
for rc in range(0, len(gender_names)):
    rn = gender_names[rc]
    print(rn)
    ...
    print('Female Attempt:', pc_female_attempt.shape[0])
    print('Female No Attempt:', pc_female_NO_attempt.shape[0])
    print('Male Attempt:', pc_male_attempt.shape[0])
    print('Male No Attempt:', pc_male_NO_attempt.shape[0])
    print()
...
for rc in range(0, len(gender_names)):
    ...
    print('Female Attempt:', nonpc_female_attempt.shape[0])
    print('Female No Attempt:', nonpc_female_NO_attempt.shape[0])
    print('Male Attempt:', nonpc_male_attempt.shape[0])
    print('Male No Attempt:', nonpc_male_NO_attempt.shape[0])
    print()
...
for rc in range(0, len(gender_names)):
    # rn = gender_names[rc]
    # print(rn)
    pc_female = pc_data[(pc_data['female'] == 1)]
    pc_male = pc_data[(pc_data['female'] == 0)]
    y_pc_female = pc_female["event90"]
    y_pc_male = pc_male["event90"]
    pc_female = pc_female.drop(columns=["event90"])
    pc_male = pc_male.drop(columns=["event90"])
    data_pc_gender['female'] = pc_female
    label_pc_gender['female'] = y_pc_female
    data_pc_gender['male'] = pc_male
    label_pc_gender['male'] = y_pc_male
    # data_pc_race.append(pc_race)
    # label_pc_race.append(y_pc)
    print(pc_female.shape)
    print(y_pc_female.shape)
    print(pc_male.shape)
    print(y_pc_male.shape)
    print()
...
for rc in range(0, len(gender_names)):
    # rn = gender_names[rc]
    # print(rn)
    nonpc_female = non_pc_data[(non_pc_data['female'] == 1)]
    nonpc_male = non_pc_data[(non_pc_data['female'] == 0)]
    y_nonpc_female = nonpc_female["event90"]
    y_nonpc_male = nonpc_male["event90"]
    nonpc_female = nonpc_female.drop(columns=["event90"])
    nonpc_male = nonpc_male.drop(columns=["event90"])
    data_nonpc_gender['female'] = nonpc_female
    label_nonpc_gender['female'] = y_nonpc_female
    data_nonpc_gender['male'] = nonpc_male
    label_nonpc_gender['male'] = y_nonpc_male
    print(nonpc_female.shape)
    print(y_nonpc_female.shape)
    print(nonpc_male.shape)
    print(y_nonpc_male.shape)
    print()
...
data_genders = [data_pc_gender, data_nonpc_gender]
# data_male = [data_pc_male, data_nonpc_male]
label_genders = [label_pc_gender, label_nonpc_gender]
# label_male = [label_pc_male, label_nonpc_male]


X_races = data_genders[test_data_idx]
y_races = label_genders[test_data_idx]
...
for i in range(0, len(gender_names_new)):
    ...
    for i in range(times):
        ...
        try:
            ...
            cm_df.to_csv('logs_MH_subset/fairness_gender/' + race_ + '_' + save_file + '_confusion_matrix.csv', index=True)
            ...
            plt.savefig('logs_MH_subset/fairness_gender/' + race_ + '_' + save_file + '_confusion_matrix.png')
