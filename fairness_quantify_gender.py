...
if test_data_idx == 0:
    ...
    for index, row in pc_data.iterrows():
        if row['female'] == 1:
            sensitive_race_ethnic.append('female')
        else:
            sensitive_race_ethnic.append('male')
elif test_data_idx == 1:
    # Non-PC data
    print("Non-PC DATA")
    # subset_df = non_pc_data[['female']]
    # print(subset_df.head(50))
    sensitive_race_ethnic = []
...
df.to_csv('logs_MH_subset/fairness_gender/' + save_file + '_quantify.csv', index=False)
