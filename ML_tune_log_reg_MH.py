...
# In[9]:
mh_data = data[data["PRIMARY_CARE_VISIT"] == 0]
# print(mh_data)
...
# In[11]:
#mh_data = mh_data[pc_column_list]
...
#
# # In[15]:
# mh_data[pc_columns_to_scale] = scale(mh_data[pc_columns_to_scale])
...
# In[18]:
mh_data = mh_data.drop(columns=["PRIMARY_CARE_VISIT","person_id","event30","event90","death30","death90","visit_mh"])
# print(mh_data)


# In[19]:
# print(mh_data.columns)
...
# says pc cols to scale, but its still MH, cols are the same, just didnt change the variable name
...
#     if sum(mh_data[i].isnull()) == 178460: #188894:
#         mh_data[i].fillna(value=0,inplace=True)
#         print(sum(mh_data[i].isnull()))
#     else:
#         mean_value=mh_data[i].mean()
#         mh_data[i].fillna(value=mean_value,inplace=True)
#         print(sum(mh_data[i].isnull()))
...
# for i in missing_columns:
#     if sum(mh_data[i].isnull()) == mh_data.shape[0]: #446893:
#         # data[i].fillna(value=0,inplace=True)
#         mh_data = mh_data.drop(columns=[i])
#         # print(sum(data[i].isnull()))
#     else:
#         if i in bin_cols:
#             mh_data[i].fillna(value=-1, inplace=True)
#         elif i in num_cols:
#             mean_value=mh_data[i].mean()
#             mh_data[i].fillna(value=mean_value,inplace=True)
#         else:
...
for i in missing_columns:
    if sum(mh_data[i].isnull()) == mh_data.shape[0]: #412045: #446893:
        # data[i].fillna(value=0,inplace=True)
        mh_data = mh_data.drop(columns=[i]) # if all rows are empty for a column, drop that column
        # print(sum(data[i].isnull()))
        print("Empty column:", i)
    else:
        mh_data[i].fillna(value=-1, inplace=True) # # if only some of rows have missing value for a column, then fill that with -1, for num or bin cols
...
# sns.heatmap(mh_data.isna().transpose(),
#             cmap="YlGnBu",
...
# In[24]:
train_X, X_test, train_y, y_test = train_test_split(mh_data, y, test_size=0.35, random_state=42, stratify = y)
...
joblib.dump(clf, 'logs_MH_subset/saved_models/log_reg_320scratch_not_balanced_MH.pkl')
print("model saved")
print()
...
joblib.dump(clf, 'logs_MH_subset/saved_models/log_reg_320scratch_balanced_MH.pkl')
print("model saved")
print()
