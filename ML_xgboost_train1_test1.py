...
# for i in missing_columns:
#     if sum(pc_data[i].isnull()) == data.shape[0]: #446893:
...
#         print(sum(non_pc_data[i].isnull()))
#     else:
#         mean_value=non_pc_data[i].mean()
#         non_pc_data[i].fillna(value=mean_value,inplace=True)
#         print(sum(non_pc_data[i].isnull()))


# for i in missing_columns:
#     if sum(non_pc_data[i].isnull()) == data.shape[0]: #446893:
#         # data[i].fillna(value=0,inplace=True)
#         non_pc_data = non_pc_data.drop(columns=[i])
#         # print(sum(data[i].isnull()))
#     else:
#         if i in bin_cols:
#             non_pc_data[i].fillna(value=-1, inplace=True)
#         elif i in num_cols:
#             mean_value=non_pc_data[i].mean()
#             non_pc_data[i].fillna(value=mean_value,inplace=True)
#         else:
...
print('Train PC, Test PC')
def objective(trial):
    n_estimators = trial.suggest_int("n_estimators", 200, 500)
    max_features = trial.suggest_categorical("max_features", ['sqrt', 'log2']) # NICK: removed 'auto' 3/7/24
    max_depth = trial.suggest_int('max_depth', 4, 50)
    criterion = trial.suggest_categorical("criterion", ['friedman_mse', 'squared_error'])
    lr = trial.suggest_float("learning_rate", 0.01, 1.0, step=0.1)
...
clf = GradientBoostingClassifier(**study.best_params).fit(X_train, y_train)
print()

print('Train PC, Test PC')
preds = clf.predict_proba(X_test.values)
test_auc = roc_auc_score(y_score=preds[:,1], y_true=y_test)
preds_y = clf.predict(X_test.values)
preds_y_val = clf.predict(X_val.values)
preds_valid = clf.predict_proba(X_val.values)
valid_auc = roc_auc_score(y_score=preds_valid[:,1], y_true=y_val)
print(f"BEST VALID AUC: {valid_auc}")
print(f"FINAL TEST AUC: {test_auc}")

roc_auc = metrics.roc_auc_score(y_test, clf.predict_proba(X_test.values)[:,1])
print("ROC AUC is: ", roc_auc)

f1 = metrics.f1_score(y_test, clf.predict(X_test.values))
print("F1 score is: ", f1)

precision = metrics.precision_score(y_test, clf.predict(X_test.values))
print("Precision score is: ", precision)

recall = metrics.recall_score(y_test, clf.predict(X_test.values))
print("Recall score is: ", recall)

print(metrics.classification_report(y_test, preds_y))
print(metrics.confusion_matrix(y_test, preds_y))
...
prec_test, recall_test, _ = metrics.precision_recall_curve(y_test, preds[:,1])
# Calculate AUC-PRC
auc_prc_test = metrics.auc(recall_test, prec_test)
print("TEST Precision-Recall AUC:", auc_prc_test)

prec_val, recall_val, _ = metrics.precision_recall_curve(y_val, preds_valid[:,1])
# Calculate AUC-PRC
...
plt.savefig('logs_MH_subset/figures/xgboost_train1_test1_pc2pc_prec_recall_curve.png')
print()
print()

# ROC-AUC curve -- Test data
fpr_curve, tpr_curve, _ = metrics.roc_curve(y_test, preds[:,1])
...
plt.savefig('logs_MH_subset/figures/xgboost_train1_test1_pc2pc_ROC_curve.png')
print('saved roc-auc curve')
print()

# save test on PC model
# clf.save_model('logs_MH_subset/saved_models/xgboost_entire_data_testPC.json')
joblib.dump(clf, 'logs_MH_subset/saved_models/xgboost_train1_test1_pc2pc.pkl')
print("model saved")
print()

# saving metrics and outputs to csv
df_pc = pd.DataFrame()
df_pc['Predicted_probs_val'] = pd.Series(preds_valid[:,1])
df_pc['Pred_y_val'] = pd.Series(preds_y_val)
df_pc['GT_y_val'] = pd.Series(y_val.values)

df_pc['Predicted_probs_test'] = pd.Series(preds[:,1])
df_pc['Pred_y_test'] = pd.Series(preds_y)
df_pc['GT_y_test'] = pd.Series(y_test.values)

df_pc['Val_ROC_AUC'] = pd.Series(valid_auc)
...
df_pc.to_csv('logs_MH_subset/saved_csvs/xgboost_train1_test1_pc2pc.csv')
...
preds = clf.predict_proba(X_test_non_pc.values)
test_auc = roc_auc_score(y_score=preds[:,1], y_true=y_test_non_pc)
preds_y = clf.predict(X_test_non_pc.values)
...
roc_auc = metrics.roc_auc_score(y_test_non_pc, clf.predict_proba(X_test_non_pc.values)[:,1])
print("ROC AUC is: ", roc_auc)

f1 = metrics.f1_score(y_test_non_pc, clf.predict(X_test_non_pc.values))
print("F1 score is: ", f1)

precision = metrics.precision_score(y_test_non_pc, clf.predict(X_test_non_pc.values))
print("Precision score is: ", precision)

recall = metrics.recall_score(y_test_non_pc, clf.predict(X_test_non_pc.values))
print("Recall score is: ", recall)

print(metrics.classification_report(y_test_non_pc, preds_y))
print(metrics.confusion_matrix(y_test_non_pc, preds_y))

tn, fp, fn, tp = metrics.confusion_matrix(y_test_non_pc, preds_y).ravel()
specificity = tn / (tn+fp)
print("Specificity score is: ", specificity)
sensitivity = tp / (tp+fn)
print("Sensitivity score is: ", sensitivity)
...
# Calculate precision-recall pairs
prec_test, recall_test, _ = metrics.precision_recall_curve(y_test_non_pc, preds[:,1])
# Calculate AUC-PRC
auc_prc_test = metrics.auc(recall_test, prec_test)
print("TEST Precision-Recall AUC:", auc_prc_test)

prec_val, recall_val, _ = metrics.precision_recall_curve(y_val_non_pc, preds_valid[:,1])
# Calculate AUC-PRC
auc_prc_val = metrics.auc(recall_val, prec_val)
print("VALID Precision-Recall AUC:", auc_prc_val)
...
# plt.title('Precision-Recall Curve')
plt.legend(loc="lower left")
plt.savefig('logs_MH_subset/figures/xgboost_train1_test1_pc2nonpc_prec_recall_curve.png')
print()
print()

# ROC-AUC curve -- Test data
fpr_curve, tpr_curve, _ = metrics.roc_curve(y_test_non_pc, preds[:,1])
...
# plt.title('Receiver Operating Characteristic')
plt.legend(loc="lower right")
plt.savefig('logs_MH_subset/figures/xgboost_train1_test1_pc2nonpc_ROC_curve.png')
print('saved roc-auc curve')
print()

# save test on PC model
# clf.save_model('logs_MH_subset/saved_models/xgboost_entire_data_testPC.json')
joblib.dump(clf, 'logs_MH_subset/saved_models/xgboost_train1_test1_pc2nonpc.pkl')
print("model saved")
print()
...
df_pc.to_csv('logs_MH_subset/saved_csvs/xgboost_train1_test1_pc2nonpc.csv')
print('metrics file saved')
print()
...
def objective(trial):
    ...
    clf.fit(X_train_non_pc, y_train_non_pc)
    preds_valid = clf.predict_proba(X_val.values)
    valid_auc = roc_auc_score(y_score=preds_valid[:, 1], y_true=y_val)
...
clf = GradientBoostingClassifier(**study.best_params).fit(X_train_non_pc, y_train_non_pc)
print()
...
# plt.title('Precision-Recall Curve')
plt.legend(loc="lower left")
plt.savefig('logs_MH_subset/figures/xgboost_train1_test1_nonpc2pc_prec_recall_curve.png')
print()
print()
...
# plt.title('Receiver Operating Characteristic')
plt.legend(loc="lower right")
plt.savefig('logs_MH_subset/figures/xgboost_train1_test1_nonpc2pc_ROC_curve.png')
print('saved roc-auc curve')
print()

# save test on PC model
# clf.save_model('logs_MH_subset/saved_models/xgboost_entire_data_testPC.json')
joblib.dump(clf, 'logs_MH_subset/saved_models/xgboost_train1_test1_nonpc2pc.pkl')
print("model saved")
print()
...
df_pc.to_csv('logs_MH_subset/saved_csvs/xgboost_train1_test1_nonpc2pc.csv')
print('metrics file saved')
print()
...
print('Train Non PC, Test Non PC')
def objective(trial):
    n_estimators = trial.suggest_int("n_estimators", 200, 500)
    max_features = trial.suggest_categorical("max_features", ['sqrt', 'log2'])  # NICK: removed 'auto' 3/7/24
    max_depth = trial.suggest_int('max_depth', 4, 50)
...
# plt.title('Precision-Recall Curve')
plt.legend(loc="lower left")
plt.savefig('logs_MH_subset/figures/xgboost_train1_test1_nonpc2nonpc_prec_recall_curve.png')
print()
print()
...
# plt.title('Receiver Operating Characteristic')
plt.legend(loc="lower right")
plt.savefig('logs_MH_subset/figures/xgboost_train1_test1_nonpc2nonpc_ROC_curve.png')
print('saved roc-auc curve')
print()

# save test on PC model
# clf.save_model('logs_MH_subset/saved_models/xgboost_entire_data_testPC.json')
joblib.dump(clf, 'logs_MH_subset/saved_models/xgboost_train1_test1_nonpc2nonpc.pkl')
print("model saved")
print()
...
df_pc.to_csv('logs_MH_subset/saved_csvs/xgboost_train1_test1_nonpc2nonpc.csv')
print('metrics file saved')
print()
