...
pth_pc_coeff_df = 'Primary care Coefficients.xlsx' # file from https://github.com/MHResearchNetwork/srpm-model
...
pc_data_renamed = pc_data

pc_data_renamed.rename(columns={'highdedectible': 'highdeductible', 'lvi_sui_att_pre5y': 'lvi_sui_att_pre5y_cumulative',
                                'raceAsian': 'raceasian', 'raceBlack': 'raceblack', 'coll_deg_It25p': 'coll_deg_lt25p',
...
print()
print('our col names:')
our320_cols = list(pc_data_renamed.columns)
print(our320_cols)
print(len(our320_cols))
print()

# loop through all 320 columns, if name not in list of 102, drop it, else leave it

cols_to_drop = []
for oc in our320_cols:
    if oc not in mhrn_col_names:
        cols_to_drop.append(oc)

pc_data = pc_data.drop(columns=cols_to_drop)

# check # cols and the names in the data set now
print(list(pc_data.columns))
print(len(pc_data.columns))
print()
...
joblib.dump(clf, 'logs_MH_subset/saved_models/log_reg_102scratch_not_balanced.pkl')
print("model saved")
print()
...
joblib.dump(clf, 'logs_MH_subset/saved_models/log_reg_102scratch_balanced.pkl')
print("model saved")
print()
