...
def train_one_trial(trial):
    d_blocks = trial.suggest_categorical("d_block", [256, 384, 512])
    n_blocks = trial.suggest_int("n_blocks", 2, 4)
    dropouts = trial.suggest_float("dropout", 0.0, 0.2)
...
df.to_csv('logs_MH_subset/new_models/NEW_statsig/MLP_tabular_' + setting + '.csv', index=False)

df2.to_csv('logs_MH_subset/new_models//NEW_statsig/MLP_tabular_' + setting + '_summary.csv', index=False)
...
torch.save(final_model, 'logs_MH_subset/new_models/saved_models/MLP_train1_test1_'+setting+".pth")
